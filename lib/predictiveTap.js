...
let htStartedAt = 0
// Freshness marker set by consumePrefire on a MATCH. clearChainTransient
// uses it to distinguish "consume just eagerly opened state in this same
// mount cycle (StrictMode/HMR re-run — do NOT clobber)" from "state is
// stale leftover from a prior navigation (clear it)". The old guard
// compared state SHAPE (currentStep === ownedStep && inAnim) which can't
// tell those apart — a back-nav from the terminal page re-mounted [iso]
// with the same shape and the stale state survived forever.
let lastConsume = { step: null, at: 0 }
const CONSUME_FRESH_MS = 800
...
//
// `ownedStep` is the step this page's consumePrefire targets. If consume
// matched that step within the last CONSUME_FRESH_MS, this is a
// StrictMode/HMR re-run of the same mount cycle — the eager-open state
// consume just populated is legitimate, so skip the clear. Anything older
// is stale leftover from a prior navigation and gets cleared. (Freshness,
// not state shape: shape-matching couldn't tell a fresh consume from a
// back-nav that re-mounted a page whose step state was left behind.)
export function clearChainTransient(reason = 'page-mount', ownedStep = null) {
  const s = getState(); if (!s) return
  if (
    ownedStep &&
    lastConsume.step === ownedStep &&
    (Date.now() - lastConsume.at) < CONSUME_FRESH_MS
  ) {
    log(`clear transient skipped (consume owns "${ownedStep}", ${Date.now() - lastConsume.at}ms ago): ${reason}`)
    return
  }
  s.inAnim = false
  s.currentStep = null
...
// Current chain step (whichever step's HT is in flight), or null if not
// mid-chain. Used by useSound to pick a step-specific haptic pattern for
// the transition-slash sound — drives the drill-rev haptic on chain hops.
...
export function consumePrefire(stepName) {
  ...
  // Mark freshness so clearChainTransient on a StrictMode/HMR re-run of
  // this same mount cycle knows not to clobber the eager-open above.
  lastConsume = { step: stepName, at: Date.now() }
