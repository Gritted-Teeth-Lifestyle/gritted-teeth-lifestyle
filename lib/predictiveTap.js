...
// Consume effects that run after this still correctly re-open inAnim
// via setInAnimation(stepName, true) in their primary handler.
export function clearChainTransient(reason = 'page-mount') {
  const s = getState(); if (!s) return
  s.inAnim = false
  s.currentStep = null
  activeHTStep = null
