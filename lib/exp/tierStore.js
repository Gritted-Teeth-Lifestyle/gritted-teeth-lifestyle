...
// One session tick per CALENDAR day per profile (Jordan 2026-07-23:
// "more than one session is being logged at a time"). Covers both
// double-tick paths: the same date stamped in two parallel cycles, and
// unlog → re-stamp of the same day. The ticked marker never clears —
// a session once earned is earned (same no-decay rule as proven bests).
// Returns the new count when it ticked, null when the day already had
// its session.
export function tickTierForDay(iso) {
  if (!iso) return null
  try {
    const key = pk(`tier-ticked-${iso}`)
