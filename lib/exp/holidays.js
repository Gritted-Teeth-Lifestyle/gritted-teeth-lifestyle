...
// True when `date` is the user's birthday (including the Feb 29 → Feb 28
// non-leap-year rollback). Exported so the set cinematic can label the
// holiday chip BIRTHDAY instead of the generic HOLIDAY.
export function isBirthdayToday(date, userDOB) {
  const dob = parseDOB(userDOB)
  if (dob == null) return false
  const month = date.getMonth()
  const day = date.getDate()
  if (dob.month === month && dob.day === day) return true
  return dob.month === 1 && dob.day === 29 &&
    month === 1 && day === 28 &&
...
export function getHolidayMultiplier(date, userDOB) {
  ...
  // Tier 1 — major (×1.5)
  if (isBirthdayToday(date, userDOB)) return 1.5
