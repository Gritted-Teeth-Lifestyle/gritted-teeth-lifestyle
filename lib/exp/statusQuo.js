...
// human, so it's a data error, not ambition):
//
//   claim ≥ 2.0                       → 'reject' (block the input)
//   claim < 1.0 (below advanced)      → honest zone, never questioned
//   jump ≤ 1.3 (gradual climb)        → believed, never taxed
//   jump > 1.3 & claim 1.0–1.35       → 'tax' ×0.75
//   jump > 1.3 & claim 1.35–1.5       → 'tax' ×0.5
//   jump > 1.3 & claim > 1.5          → 'tax' ×0.25
//
...
const REJECT_CLAIM = 2.0
const JUMP_TOLERANCE = 1.3
const CLIMB_LO = 0.9
const CLIMB_HI = 1.1
...
export function assessSet({ exercise, weight, reps, bodyweight, provenBest }) {
  ...
  // Honest zone: below the advanced standard, no questions ever.
  // Above it, a gradual climber (jump ≤ 1.3 vs proven history) is
  // believed; only claims that outrun their own record get taxed.
  const taxed = claim >= 1.0 && jump > JUMP_TOLERANCE
  if (taxed) {
    out.kind = 'tax'
...
// Convenience: assessment with provenBest read from storage.
...
// Total training days on this profile — days with at least one logged
// set, across ALL cycles (including deleted ones): every trained day
// leaves a pk('xpLog-{cycleId}-{iso}') key behind, so this is a prefix
// scan, retroactive over the profile's whole history. Feeds the WAR
// RECORD stat and the earned experience tier.
