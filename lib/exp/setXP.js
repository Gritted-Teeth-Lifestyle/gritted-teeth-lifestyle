...
// is_king_compound, is_isolation_override, and is_compound_override
// take precedence. is_compound_override exists for multi-joint moves
// whose muscles all live in ONE region (e.g. overhead press =
// shoulders+triceps = ARMS only) that the auto rule would wrongly
// call isolation.
export function classifyExercise(exercise) {
  ...
  if (exercise?.is_compound_override) return 'compound'
...
export function calculateSetXP(set, exercise, user, runtimeState = {}) {
  ...
  // STATUS QUO honesty layer (lib/exp/statusQuo.js): CLIMB bonus (1.1)
  // or plausibility tax (0.75/0.5/0.25). Scales the whole set — base,
  // heavy bonus, and every contribution — so there's no side door.
  const statusQuoMult   = runtimeState.statusQuoMult ?? 1.0
  const statusQuoKind   = runtimeState.statusQuoKind ?? 'none'
  ...
  // Display scale: shipped EXP numbers are 1/100 of the raw math so the UI
  // reads as RPG-feel small integers (set ≈ 10-30 EXP) instead of 4-digit
  // numbers. Only XP-valued fields are scaled — multipliers, ratios, and
  // region weights stay as fractions. Level thresholds in getLevelInfo are
  // scaled to match (150 + level × 10).
  const SCALE = 1 / 100
  ...
  return {
    ...
    reps,
    weight,
    rawBase: rawBase * SCALE,
    baseXP: stackBase * SCALE,    // "normalized base" the cinematic shows
    normFactor,
    heavyLiftBonus: heavyLiftBonus * SCALE,
    combinedFactor,
    classification: cls,
    classMult,
    ...
    holidayMult,
    statusQuoMult,
    statusQuoKind,                // 'climb' | 'tax' | 'none' — drives the cinematic chip
    totalXP: totalXP * SCALE,     // R2 sum of contributions; consistency deferred
