...
// distributed by snapshot.regionWeights. Consistency-credit entries are
// distributed evenly across the regions touched by the day's set
// snapshots (proportional to per-region XP from those sets); if no set
// snapshots exist for the day, the consistency credit doesn't bind to
// any region.
export function sumDayRegionXP(cycleId, iso) {
  ...
  // Distribute consistency_credit proportionally to per-region setXP totals.
  if (consistencyCredit > 0 && totalSetXP > 0) {
    for (let i = 0; i < 5; i++) {
      regionXP[i] += consistencyCredit * (regionXP[i] / totalSetXP)
    }
  }
  return regionXP
}
