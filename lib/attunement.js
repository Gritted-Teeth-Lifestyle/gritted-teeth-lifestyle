...
/**
 * Auto-attune one exercise per muscle per workout day. `dayMuscleMap` is
 * `{ dayId: [muscle1, muscle2, ...] }` — each muscle becomes a chip.
 * `pickExerciseFor(muscle)` returns the exercise id to attach.
...
export function autoAttuneAll(cycleId, dayMuscleMap, pickExerciseFor, { overwrite = false } = {}) {
  ...
  for (const [dayId, muscles] of Object.entries(dayMuscleMap || {})) {
    ...
    for (const muscle of list) {
      if (!muscle) continue
      const exerciseId = pickExerciseFor(muscle)
